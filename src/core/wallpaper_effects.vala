...
namespace Singularity {
    // Display-time image effects backed by a persistent, bounded cache. The
    // caller supplies a stable source identity; dimensions and effect
    // parameters are added here so differently-sized renders never collide.
    public class WallpaperEffects : Object {
        ...
        public Gdk.Pixbuf apply_blur(Gdk.Pixbuf source, string source_cache_key, int radius) {
            ...
            var cached = get(key); if (cached != null) return cached;
            var output = source.copy();
            // Three box passes are a close Gaussian approximation and keep
            // runtime linear in radius via a sliding window.
